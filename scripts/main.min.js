var log="",VfrRepArray,CurrentAerodromeArray,OfficialAerodromeArray,ZZZZFieldArray,gPosition_lat,gPosition_lon,flightPlanLink,gCalculatedFlyingTimeStr,gSelectedFlyingTimeStr,dateOfUpdateStr="2014-11-13T00:00:00Z",AerodromesFilename_before_13NOV2014="aerodromes_19SEP2013.xml",VFRPortFilename_before_13NOV2014="EF_VFRREP_19SEP2013.xml",ZZZZFieldsFilename_before_13NOV2014="zzzz_fields_18JAN2014.xml",VFRPortFilename="EF_VFRREP_13NOV2014.xml",AerodromesFilename="aerodromes_13NOV2014.xml",ZZZZFieldsFilename=
"zzzz_fields_13NOV2014.xml",UNOFFICIAL_AERODROME="ZZZZ",UNOFFICIAL_AERODROME_INDEX=0,BY_PHONE_ON_GROUND_STR="PHONE ACC ",BY_PHONE_ON_GROUND_STR_WITH_PHONE="PHONE ACC 032865172 ",BY_RTF_ON_AIR_STR="RTF ACC ",BY_TWR="- (torni)",NOTE_TO_ADD_ACC_FREQ_STR="LIS\u00c4\u00c4_ACC_JAKSO",overrideFlightTime=!0;function debug_log(a){log+=a+"<br>";document.getElementById("log").innerHTML=log}function debug_timestamp(a){}function getFlightTimeOverride(){return overrideFlightTime}
...
function handlePositionUpdateCallback(a){handlePositionUpdate(a.coords.latitude,a.coords.longitude);a=1E3>a.coords.accuracy?"("+Math.round(a.coords.accuracy)+"m)":"("+Math.round(a.coords.accuracy/1E3)+"km)";document.getElementById("locationUpdateButtonText").innerHTML="Sijainti "+a}
...
function aerodromeXmlResponseHandler(){if(this.readyState==this.DONE||4==this.readyState)200==this.status&&null!==this.responseXML?(debug_timestamp("aerodromeXml loaded"),aerodromeReadyHandler(this.responseXML)):debug_log("ERROR: aerodromeXmlResponseHandler - error: status="+this.status)}
...
a[b].attributes.getNamedItem("lat").value,f=a[b].attributes.getNamedItem("lon").value;VfrRepArray.push(new Waypoint(c,d,f))}debug_log("=> "+VfrRepArray.length+" VFR ilmoittautumispistett\u00e4.");loadXMLDoc(ZZZZFieldsFilename,zzzzFieldXmlResponseHandler)}
function aerodromeReadyHandler(a){debug_timestamp("aerodromeReadyHandler start");try{var b=a.getElementsByTagName("gpx");0<b.length&&debug_log("Lentokentt\u00e4tiedot p\u00e4iv\u00e4tty: "+b[0].attributes.getNamedItem("created").value)}catch(c){debug_log("Could not read xml date: "+c)}try{var d=a.getElementsByTagName("aerodrome");OfficialAerodromeArray=[];OfficialAerodromeArray.push(new Aerodrome("Ep\u00e4virallinen",UNOFFICIAL_AERODROME,"000000N","0000000E","no",NOTE_TO_ADD_ACC_FREQ_STR,[]));for(a=
0;a<d.length;++a){for(var e=d[a].getElementsByTagName("name")[0].childNodes[0].nodeValue,f=d[a].getElementsByTagName("icao")[0].childNodes[0].nodeValue,g=d[a].getElementsByTagName("atc")[0].childNodes[0].nodeValue,h=d[a].getElementsByTagName("acc")[0].childNodes[0].nodeValue.replace("."," "),k=d[a].attributes.getNamedItem("lat").value,l=d[a].attributes.getNamedItem("lon").value,m=d[a].getElementsByTagName("vfrpoint"),b=[],n=0;n<m.length;++n)b.push(m[n].childNodes[0].nodeValue);OfficialAerodromeArray.push(new Aerodrome(e,
f,k,l,g,h,b))}}catch(p){alert(p)}debug_log("=> "+OfficialAerodromeArray.length+" lentokentt\u00e4\u00e4.");loadXMLDoc(VFRPortFilename,vfrPortXmlResponseHandler);debug_timestamp("aerodromeReadyHandler end")}
function zzzzFieldReadyHandler(a){try{var b=a.getElementsByTagName("gpx");0<b.length&&debug_log("Ep\u00e4virallisten lentopaikkojen tiedot p\u00e4iv\u00e4tty: "+b[0].attributes.getNamedItem("created").value)}catch(c){debug_log("Could not read xml date: "+c)}try{var d=a.getElementsByTagName("aerodrome");ZZZZFieldArray=[];for(a=0;a<d.length;++a){var e=d[a].getElementsByTagName("name")[0].childNodes[0].nodeValue,f=d[a].getElementsByTagName("icao")[0].childNodes[0].nodeValue,g=d[a].getElementsByTagName("atc")[0].childNodes[0].nodeValue,
h=d[a].getElementsByTagName("acc")[0].childNodes[0].nodeValue.replace("."," "),k=d[a].attributes.getNamedItem("lat").value,l=d[a].attributes.getNamedItem("lon").value;d[a].getElementsByTagName("vfrpoint");ZZZZFieldArray.push(new Aerodrome(e,f,k,l,g,h,[]))}}catch(m){alert(m)}debug_log("=> "+ZZZZFieldArray.length+" ep\u00e4virallista lentopaikkaa.");continueIfEverythingIsReady()}function removeOptions(a){for(;0<a.length;)a.remove(0)}
...
function onLoad(){var a=new Date(dateOfUpdateStr);(new Date).getTime()<a.getTime()&&(VFRPortFilename=VFRPortFilename_before_13NOV2014,AerodromesFilename=AerodromesFilename_before_13NOV2014,ZZZZFieldsFilename=ZZZZFieldsFilename_before_13NOV2014);debug_timestamp("aerodromeXml load start");loadXMLDoc(AerodromesFilename,aerodromeXmlResponseHandler);navigator.geolocation?updateCurrentLocation():(debug_log("Geolocation ei ole tuettu selaimessa."),fakeCurrentLocation(),document.getElementById("locationUpdateButtonText").innerHTML=
"Ei sijaintia");updateFromLocalStorage();onChangeFlightLv();updateStoredPlanList();initStoredPlanPageHandler()}function fakeCurrentLocation(){debug_log("Todellista sijaintia ei saada. K\u00e4ytet\u00e4\u00e4n sijaintina Tamperetta...");gPosition_lat=61.49816;gPosition_lon=23.761055;handlePositionUpdate(gPosition_lat,gPosition_lon)}
...
function timeNext5mins(a){return a=5*Math.ceil(a/5)}function convertFlyingTimeToString(a){var b=a%60;a=(a-b)/60;var c="";10>a&&(c+="0");c+=a;10>b&&(c+="0");return c+b}function getDistanceInCurrentUnit(a){var b=document.getElementById("aircraftSpeedUnit").value;switch(b){case "K":return Number(a);case "N":return 1E3*Number(a)/1852;default:debug_log("Error: getDistanceInCurrentUnit() - unknown aircraftSpeedUnit: "+b)}return"unknown"}
...
else{var d=document.getElementById("departure").value,e=document.getElementById("route_departure_rep").value,f=document.getElementById("route_destination_rep").value,g=document.getElementById("destination").value,a=[];a.push(getAerodromeByIndex(d));a.push(getVfrWayPointByName(e));a.push(getVfrWayPointByName(f));a.push(getAerodromeByIndex(g));e='<table data-role="table" id="flyingTimeTable" class="table-stripe"><thead><tr class="ui-bar-a">';e+='<th data-priority="persist">paikka</th>';e+='<th data-priority="2">'+
