var log="Ohjelmakoodi p\u00e4iv\u00e4tty: 2015-05-21<br>",VfrRepArray,CurrentAerodromeArray,OfficialAerodromeArray,ZZZZFieldArray,gPosition_lat,gPosition_lon,flightPlanLink,gCalculatedFlyingTimeStr,gSelectedFlyingTimeStr,dateOfUpdateStr="2015-07-23T00:00:00Z",VFRPortFilename_before_change="EF_VFRREP_13NOV2014.xml",AerodromesFilename_before_change="aerodromes_28MAY2015.xml",ZZZZFieldsFilename_before_change="zzzz_fields_28MAY2015.xml",VFRPortFilename="EF_VFRREP_13NOV2014.xml",AerodromesFilename="aerodromes_23JUL2015.xml",
ZZZZFieldsFilename="zzzz_fields_28MAY2015.xml",UNOFFICIAL_AERODROME="ZZZZ",UNOFFICIAL_AERODROME_INDEX=0,BY_PHONE_ON_GROUND_STR="PHONE ACC ",BY_PHONE_ON_GROUND_STR_WITH_PHONE="PHONE ACC 032865172 ",BY_RTF_ON_AIR_STR="RTF ACC ",BY_TWR="- (torni)",NOTE_TO_ADD_ACC_FREQ_STR="LIS\u00c4\u00c4_ACC_JAKSO",PYROTECHNICS_NOTE_REMARKS="rakettipelastusvarjo",OPENAVIATIONDATA_APIKEY="mkXQV7UpiiBgOVjxbTzioYfpOlNVtEtg",overrideFlightTime=!0,total_startup_start_time=0,aerodromeXml_start_time=0;
function debug_log(a){log+=a+"<br>";document.getElementById("log").innerHTML=log}function debug_timestamp_start(a){return(new Date).getTime()}function debug_timestamp_ready(a,b){(new Date).getTime()}function getFlightTimeOverride(){return overrideFlightTime}
...
function vfrPortReadyHandler(a){var b=debug_timestamp_start("vfrPortReadyHandler");try{var c=a.getElementsByTagName("metadata");if(0<c.length){var d=c[0].getElementsByTagName("time")[0].childNodes[0].nodeValue,e=new Date(d);debug_log("VFR ilmoittautumispisteet p\u00e4iv\u00e4tty: "+e.getFullYear()+"-"+(e.getMonth()+1)+"-"+e.getDate())}}catch(f){debug_log("Could not read xml date: "+f)}a=a.getElementsByTagName("wpt");VfrRepArray=[];for(c=0;c<a.length;++c)if(d=a[c].getElementsByTagName("name")[0].childNodes[0].nodeValue,
"Compulsory"==a[c].getElementsByTagName("desc")[0].childNodes[0].nodeValue){var e=a[c].attributes.getNamedItem("lat").value,g=a[c].attributes.getNamedItem("lon").value;VfrRepArray.push(new Waypoint(d,e,g))}debug_log("=> "+VfrRepArray.length+" VFR ilmoittautumispistett\u00e4.");debug_timestamp_ready("vfrPortReadyHandler",b);loadXMLDoc(ZZZZFieldsFilename,zzzzFieldXmlResponseHandler)}
function aerodromeReadyHandler(a){var b=debug_timestamp_start("aerodromeReadyHandler");try{var c=a.getElementsByTagName("gpx");0<c.length&&debug_log("Lentokentt\u00e4tiedot p\u00e4iv\u00e4tty: "+c[0].attributes.getNamedItem("created").value)}catch(d){debug_log("Could not read xml date: "+d)}try{var e=a.getElementsByTagName("aerodrome");OfficialAerodromeArray=[];OfficialAerodromeArray.push(new Aerodrome("Ep\u00e4virallinen",UNOFFICIAL_AERODROME,"000000N","0000000E","no",NOTE_TO_ADD_ACC_FREQ_STR,[]));
for(a=0;a<e.length;++a){for(var f=e[a].getElementsByTagName("name")[0].childNodes[0].nodeValue,g=e[a].getElementsByTagName("icao")[0].childNodes[0].nodeValue,h=e[a].getElementsByTagName("atc")[0].childNodes[0].nodeValue,k=e[a].getElementsByTagName("acc")[0].childNodes[0].nodeValue.replace("."," "),l=e[a].attributes.getNamedItem("lat").value,m=e[a].attributes.getNamedItem("lon").value,n=e[a].getElementsByTagName("vfrpoint"),c=[],p=0;p<n.length;++p)c.push(n[p].childNodes[0].nodeValue);OfficialAerodromeArray.push(new Aerodrome(f,
g,l,m,h,k,c))}}catch(q){alert(q)}debug_log("=> "+OfficialAerodromeArray.length+" lentokentt\u00e4\u00e4.");debug_timestamp_ready("aerodromeReadyHandler",b);loadXMLDoc(VFRPortFilename,vfrPortXmlResponseHandler)}
function zzzzFieldReadyHandler(a){var b=debug_timestamp_start("zzzzFieldReadyHandler");try{var c=a.getElementsByTagName("gpx");0<c.length&&debug_log("Ep\u00e4virallisten lentopaikkojen tiedot p\u00e4iv\u00e4tty: "+c[0].attributes.getNamedItem("created").value)}catch(d){debug_log("Could not read xml date: "+d)}try{var e=a.getElementsByTagName("aerodrome");ZZZZFieldArray=[];for(a=0;a<e.length;++a){var f=e[a].getElementsByTagName("name")[0].childNodes[0].nodeValue,g=e[a].getElementsByTagName("icao")[0].childNodes[0].nodeValue,
h=e[a].getElementsByTagName("atc")[0].childNodes[0].nodeValue,k=e[a].getElementsByTagName("acc")[0].childNodes[0].nodeValue.replace("."," "),l=e[a].attributes.getNamedItem("lat").value,m=e[a].attributes.getNamedItem("lon").value;e[a].getElementsByTagName("vfrpoint");ZZZZFieldArray.push(new Aerodrome(f,g,l,m,h,k,[]))}}catch(n){alert(n)}debug_log("=> "+ZZZZFieldArray.length+" ep\u00e4virallista lentopaikkaa.");debug_timestamp_ready("zzzzFieldReadyHandler",b);continueIfEverythingIsReady()}
...
a;try{$("#pyrotechnicsOnBoard").flipswitch("refresh")}catch(e){}a=localStorage.getItem("zzzzFieldsEnabled");null!==a&&(document.getElementById("zzzzFieldsEnabled").value=a);updateAircraftSelectionList()}function isValid(a){return void 0===a||0>=a.length?!1:!0}
...
function onLoad(){total_startup_start_time=debug_timestamp_start("onLoad");var a=new Date(dateOfUpdateStr);(new Date).getTime()<a.getTime()&&(VFRPortFilename=VFRPortFilename_before_change,AerodromesFilename=AerodromesFilename_before_change,ZZZZFieldsFilename=ZZZZFieldsFilename_before_change);aerodromeXml_start_time=debug_timestamp_start("aerodromeXml load start");loadXMLDoc(AerodromesFilename,aerodromeXmlResponseHandler);navigator.geolocation?updateCurrentLocation():(debug_log("Geolocation ei ole tuettu selaimessa."),
fakeCurrentLocation(),document.getElementById("locationUpdateButtonText").innerHTML="Ei sijaintia");updateFromLocalStorage();onChangeFlightLv();updateStoredPlanList();initStoredPlanPageHandler();hideOFPContainer()}function fakeCurrentLocation(){debug_log("Todellista sijaintia ei saada. K\u00e4ytet\u00e4\u00e4n sijaintina Tamperetta...");gPosition_lat=61.49816;gPosition_lon=23.761055;handlePositionUpdate(gPosition_lat,gPosition_lon)}
function updateCurrentLocation(){navigator.geolocation&&(document.getElementById("locationUpdateButtonText").innerHTML="Sijainti...",navigator.geolocation.getCurrentPosition(handlePositionUpdateCallback,handlePositionError))}function sortNearestFirst(a,b,c){for(var d=0;d<a.length;++d)a[d].calcDistanceFrom(b,c);a.sort(waypointDistanceSort);return a}
...
function requestOpenDataDest(a){$("#metarDestId").text("");$("#tafDestId").text("");a!==UNOFFICIAL_AERODROME&&(getMetar(a,"metarDestId"),getTaf(a,"tafDestId"))}function getScaleFactor(a){a=max(x)-min(x);var b=max(y)-min(y);max(a,b)}function scaleCoordinates(){}
function drawVfrPoint(a,b,c,d){a.beginPath();a.moveTo(b,c-5.7735);a.lineTo(b+5,c+2.8868);a.lineTo(b-5,c+2.8868);a.lineTo(b,c-5.7735);a.fill();a.closePath();a.stroke();a.font="14px Arial";a.fillStyle="black";a.textAlign="center";a.fillText(d,b,c-10)}function drawAirfield(a,b,c,d){a.fillStyle="white";a.beginPath();a.arc(b,c,7,0,2*Math.PI);a.closePath();a.stroke();a.fillStyle="black";a.font="14px Arial";a.textAlign="center";a.fillText(d,b,c-14)}
function drawArrow(a,b,c,d,e,f){a.moveTo(b,c);a.lineTo(d,e);a.stroke()}function updateMapData(){debug_log("updateMapData");var a=document.getElementById("mapCancas").getContext("2d");drawArrow(a,110,110,120,170,"151");drawArrow(a,120,170,70,220,"221");drawArrow(a,70,220,30,250,"226");drawVfrPoint(a,120,170,"VIILA");drawAirfield(a,110,110,"EFTP");drawVfrPoint(a,70,220,"MANSE");drawAirfield(a,30,250,"EFTU")}
...
function getDistanceInCurrentUnit(a){var b=document.getElementById("aircraftSpeedUnit").value;switch(b){case "K":return Number(a);case "N":return 1E3*Number(a)/1852;default:debug_log("Error: getDistanceInCurrentUnit() - unknown aircraftSpeedUnit: "+b)}return"unknown"}
function createFlyingTimeTable(){updateAircraftIdentificationToMainpage();var a=document.getElementById("aircraftSpeedUnit").value,b=document.getElementById("aircraftSpeed").value,c="";switch(a){case "K":b=Number(b);c="km";break;case "N":b=1852*Number(b)/1E3;c="NM";break;default:debug_log("Error: createFlyingTimeTable() - unknown aircraftSpeedUnit: "+a),b=Number(b)}if(0==b)document.getElementById("flyingTime").innerHTML='<a href="#page-plan-settings">T\u00e4yt\u00e4 Perustiedot sivulle puuttuvat tiedot!</a>';
else{var d=document.getElementById("departure").value,e=document.getElementById("route_departure_rep").value,f=document.getElementById("route_destination_rep").value,g=document.getElementById("destination").value,a=[];a.push(getAerodromeByIndex(d));a.push(getVfrWayPointByName(e));a.push(getVfrWayPointByName(f));a.push(getAerodromeByIndex(g));for(var e='<table data-role="table" id="flyingTimeTable" class="table-stripe"><thead><tr class="ui-bar-a">',e=e+'<th data-priority="persist">paikka</th>',e=e+
('<th data-priority="2">'+c+"</th>"),e=e+'<th data-priority="1">min</th>',e=e+"</tr></thead><tbody>",d="<tr>"+("<td>"+a[0].icao+" ("+a[0].name+")</td>"),d=d+"<td>0</td><td>0</td>",d=d+"</tr>",e=e+d,h=a[0],f=0,g=1;g<a.length;++g)if(null!==a[g]){if(h.lat!=a[g].lat||h.lon!=a[g].lon)var d="<tr>",k=addPrefixDigits(Bearing(h.lat,h.lon,a[g].lat,a[g].lon).toString(),0,3),d=d+("<td><center>"+k+"&deg;</center></td>"),d=d+"<td></td>",d=d+"<td></td>",d=d+"</tr>",e=e+d;h=calcDistanceFrom(h.lat,h.lon,a[g].lat,
a[g].lon);f+=h;d="<tr>";d=void 0===a[g].icao?d+("<td>"+a[g].name+"</td>"):d+("<td>"+a[g].icao+" ("+a[g].name+")</td>");d+="<td>"+Math.round(getDistanceInCurrentUnit(h))+"</td>";d+="<td>"+getTimeFromDistance(h,b)+"</td>";d+="</tr>";e+=d;h=a[g]}d="<tr><th>Yhteens\u00e4</th>";d+="<th>"+Math.round(getDistanceInCurrentUnit(f))+" "+c+"</th>";d+="<th>"+getTimeFromDistance(f,b)+" min</th>";d+="</tr>";e+=d;document.getElementById("flyingTime").innerHTML=e;b=getTimeFromDistance(f,b);b=timeNext5mins(b);10>b&&
(b=10);gCalculatedFlyingTimeStr=convertFlyingTimeToString(b);setFlightTimeOverride(!1)}}function convertScandinavianLetters(a){a=a.replace(/[\u00e4\u00e5]/g,"a");a=a.replace(/[\u00c4\u00c5]/g,"A");a=a.replace(/\u00f6/g,"o");return a=a.replace(/\u00d6/g,"O")}
function updateFlyingTimeValues(){gSelectedFlyingTimeStr=gCalculatedFlyingTimeStr;if(getFlightTimeOverride()){for(var a="",b=document.getElementsByName("flightTimeHour"),c=0;c<b.length;c++)if(1==b[c].checked){a=b[c].value;break}b=document.getElementsByName("flightTimeMin");for(c=0;c<b.length;c++)if(1==b[c].checked){a+=b[c].value;break}if(4!==a.length)return alert("Itse m\u00e4\u00e4riteltyyn lentoaikaan t\u00e4ytyy antaa sek\u00e4 tunnit ett\u00e4 minuutit!"),!1;if(Number(a)<Number(gSelectedFlyingTimeStr))return alert("Itse m\u00e4\u00e4ritellyksi lentoajaksi ei voi antaa v\u00e4hemp\u00e4\u00e4 kuin laskelmassa on laskettu!"),
!1;gSelectedFlyingTimeStr=a}}
function updateFlightPlanLink(){for(var a=document.getElementsByName("departureTimeHour"),b=0,c=0;c<a.length;c++)if(a[c].checked){b=Number(a[c].value);break}for(var a=document.getElementsByName("departureTimeMin"),d=0,c=0;c<a.length;c++)if(a[c].checked){d=Number(a[c].value);break}if(0===b+d)return alert("L\u00e4ht\u00f6aika ei voi olla heti.\nMuuta l\u00e4ht\u00f6aika j\u00e4rkev\u00e4ksi."),!1;c=new Date;a="";c.setMinutes(timeNext5mins(c.getMinutes())+d);c.setHours(c.getHours()+b);b=c.getUTCHours();
d=c.getUTCMinutes();10>b&&(a+="0");a+=b;10>d&&(a+="0");a+=d;b="&id="+document.getElementById("aircraftIdentification").value;b+="&rules=V&type=G";b+="&aircraft="+document.getElementById("aircraftType").value;b+="&cat=L";b+="&equipment="+document.getElementById("equipment").value;b+="&ssr="+document.getElementById("ssr").value;b+="&ad="+getAerodromeByIndex(document.getElementById("departure").value).icao;b+="&time="+a;b+="&spd="+document.getElementById("aircraftSpeedUnit").value;b+="&speed="+document.getElementById("aircraftSpeed").value;
