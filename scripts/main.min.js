...
function calcDistanceFrom(a,b,c,d){var e=toRad(c-a);b=toRad(d-b);a=toRad(a);c=toRad(c);a=Math.sin(e/2)*Math.sin(e/2)+Math.sin(b/2)*Math.sin(b/2)*Math.cos(a)*Math.cos(c);return 12742*Math.atan2(Math.sqrt(a),Math.sqrt(1-a))}Waypoint.prototype.getDistance=function(){return this.distance};Waypoint.prototype.setDistance=function(a){this.distance=a};Waypoint.prototype.calcDistanceFrom=function(a,b){this.distance=calcDistanceFrom(this.lat,this.lon,a,b)};Aerodrome.prototype=new Waypoint;
Aerodrome.prototype.constructor=Aerodrome;function Aerodrome(a,b,c,d,e,f,g){Waypoint.call(this,a,DMS_to_Decimal(c),DMS_to_Decimal(d));this.latStr=DMS_to_DM(c);this.lonStr=DMS_to_DM(d);this.icao=b;this.atc=e;this.acc=f;this.vfrPoints=g}function waypointDistanceSort(a,b){return a.distance-b.distance}
function vfrPortXmlResponseHandler(){if(this.readyState==this.DONE||4==this.readyState)200==this.status&&null!==this.responseXML?vfrPortReadyHandler(this.responseXML):debug_log("ERROR: vfrPortXmlResponseHandler - error: status="+this.status)}
...
for(a=0;a<d.length;++a){for(var e=d[a].getElementsByTagName("name")[0].childNodes[0].nodeValue,f=d[a].getElementsByTagName("icao")[0].childNodes[0].nodeValue,g=d[a].getElementsByTagName("atc")[0].childNodes[0].nodeValue,h=d[a].getElementsByTagName("acc")[0].childNodes[0].nodeValue.replace("."," "),k=d[a].attributes.getNamedItem("lat").value,l=d[a].attributes.getNamedItem("lon").value,m=d[a].getElementsByTagName("vfrpoint"),c=[],p=0;p<m.length;++p)c.push(m[p].childNodes[0].nodeValue);OfficialAerodromeArray.push(new Aerodrome(e,
f,k,l,g,h,c))}}catch(n){alert(n)}debug_log("=> "+OfficialAerodromeArray.length+" lentokentt\u00e4\u00e4.");debug_timestamp_ready("aerodromeReadyHandler",b);loadXMLDoc(VFRPortFilename,vfrPortXmlResponseHandler)}
function zzzzFieldReadyHandler(a){var b=debug_timestamp_start("zzzzFieldReadyHandler");try{var c=a.getElementsByTagName("gpx");0<c.length&&debug_log("Ep\u00e4virallisten lentopaikkojen tiedot p\u00e4iv\u00e4tty: "+c[0].attributes.getNamedItem("created").value)}catch(m){debug_log("Could not read xml date: "+m)}try{var d=a.getElementsByTagName("aerodrome");ZZZZFieldArray=[];for(a=0;a<d.length;++a){var e=d[a].getElementsByTagName("name")[0].childNodes[0].nodeValue,f=d[a].getElementsByTagName("icao")[0].childNodes[0].nodeValue,
g=d[a].getElementsByTagName("atc")[0].childNodes[0].nodeValue,h=d[a].getElementsByTagName("acc")[0].childNodes[0].nodeValue.replace("."," "),k=d[a].attributes.getNamedItem("lat").value,l=d[a].attributes.getNamedItem("lon").value;d[a].getElementsByTagName("vfrpoint");ZZZZFieldArray.push(new Aerodrome(e,f,k,l,g,h,[]))}}catch(m){alert(m)}debug_log("=> "+ZZZZFieldArray.length+" ep\u00e4virallista lentopaikkaa.");debug_timestamp_ready("zzzzFieldReadyHandler",b);continueIfEverythingIsReady()}
...
function updateCurrentLocation(){navigator.geolocation&&(document.getElementById("locationUpdateButtonText").innerHTML="Sijainti...",$("#location-button").removeClass("location-not-available").addClass("location-waiting"),navigator.geolocation.getCurrentPosition(handlePositionUpdateCallback,handlePositionError))}
function sortNearestFirst(a,b,c){for(var d=getVfrWayPointsByName(c.vfrPoints),e=0;e<a.length;++e){var f=-1,g=calcDistanceFrom(a[e].lat,a[e].lon,b.lat,b.lon);if(0==d.length){var h=calcDistanceFrom(a[e].lat,a[e].lon,c.lat,c.lon);a[e].setDistance(g+h)}else for(var k=0;k<d.length;++k){var h=calcDistanceFrom(a[e].lat,a[e].lon,d[k].lat,d[k].lon),l=calcDistanceFrom(d[k].lat,d[k].lon,c.lat,c.lon);if(f>g+h+l||-1==f)f=g+h+l,a[e].setDistance(f)}}a.sort(waypointDistanceSort)}
function fillVfrWaypointSelectionData(a,b,c){removeOptions(a);var d=getVfrWayPointsByName(b.vfrPoints);if(0!=d.length){sortNearestFirst(d,b,c);appendOption(a,"-","");for(var e=0;e<d.length;++e)appendOption(a,d[e].name,d[e].name);b!==c&&(a.selectedIndex=1)}}function updatePlanActivationMethods(a,b){removeOptions(a);"yes"==b.atc&&appendOption(a,BY_TWR,"twr");appendOption(a,"Puhelimella maassa (ACC)","phoneOnGound");appendOption(a,"Radiolla ilmassa (ACC, "+b.acc+")","rtfOnAir")}
