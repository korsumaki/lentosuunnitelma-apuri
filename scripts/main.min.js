var log="",VfrRepArray,CurrentAerodromeArray,OfficialAerodromeArray,ZZZZFieldArray,gPosition_lat,gPosition_lon,flightPlanLink,gCalculatedFlyingTimeStr,gSelectedFlyingTimeStr,VFRPortFilename="17NOV2011.xml",AerodromesFilename="aerodromes.xml",ZZZZFieldsFilename="zzzz_fields.xml",UNOFFICIAL_AERODROME="ZZZZ",UNOFFICIAL_AERODROME_INDEX=0,BY_PHONE_ON_GROUND_STR="PHONE ACC 032865172 ",BY_RTF_ON_AIR_STR="RTF ACC ",BY_TWR="- (torni)",overrideFlightTime=!0;
function debug_log(a){log+=a+"<br>";document.getElementById("log").innerHTML=log}function getFlightTimeOverride(){return overrideFlightTime}
...
return void 0===gCalculatedFlyingTimeStr?!1:isValid(a)&&isValid(b)&&isValid(c)&&isValid(e)&&isValid(d)&&isValid(f)&&isValid(g)&&isValid(h)&&isValid(j)&&isValid(k)?!0:!1}catch(l){return!1}}function getPlansFromStorage(){var a=localStorage.getItem("storedPlans");return null==a?[]:JSON.parse(a)}function addPlanToStorage(a){var b=getPlansFromStorage();b.push(a);localStorage.setItem("storedPlans",JSON.stringify(b))}
...
function initStoredPlanPageHandler(){$("#page-stored-plan").on("pagebeforeshow",function(){var a=getPlansFromStorage(),b=localStorage.getItem("selectedStoredPlanIndex");$("#storedPlanAircraftIdentification").val(a[b].identification);$("#storedPlanDeparture").val(a[b].departure);$("#storedPlanDestination").val(a[b].destination);$("#storedPlanRoute").val(a[b].dep_route+" "+a[b].route+" "+a[b].dest_route);$("#storedPlanTakeoffTime").val(getTimeIn4digits(new Date(a[b].takeoff_time))+" (sa)");$("#storedPlanFlightTime").val(a[b].flight_time);
$("#storedPlanDepMethod").val(a[b].activation_method);$("#storedPlanArrMethod").val(a[b].completion_method);a[b].activation_method!==BY_PHONE_ON_GROUND_STR&&a[b].completion_method!==BY_PHONE_ON_GROUND_STR?$("#callAccButton").hide():$("#callAccButton").show()})}
function onLoad(){loadXMLDoc(AerodromesFilename,aerodromeXmlResponseHandler);navigator.geolocation?updateCurrentLocation():(debug_log("Geolocation ei ole tuettu selaimessa."),fakeCurrentLocation());updateFromLocalStorage();onChangeFlightLv();updateStoredPlanList();initStoredPlanPageHandler()}
...
document.getElementById("pilotTel").value;b+="&filed="+document.getElementById("pilot").value;b=convertScandinavianLetters(b);flightPlanLink="http://www.finavia.fi/C/flightplan/efpl_lomake/"+b;return!0}
function getPlanForStoring(){debug_log("getPlanForStoring");for(var a={},b=document.getElementsByName("departureTimeHour"),c=0,d=0;d<b.length;d++)if(b[d].checked){c=Number(b[d].value);break}for(var e=document.getElementsByName("departureTimeMin"),d=b=0;d<e.length;d++)if(e[d].checked){b=Number(e[d].value);break}if(0===c+b)return alert("L\u00e4ht\u00f6aika ei voi olla heti.\nMuuta l\u00e4ht\u00f6aika j\u00e4rkev\u00e4ksi."),!1;d=new Date;d.setMinutes(timeNext5mins(d.getMinutes())+b);d.setHours(d.getHours()+
c);d.getUTCHours();d.getUTCMinutes();"phoneOnGound"==document.getElementById("planActivationMethod").value?a.activation_method=BY_PHONE_ON_GROUND_STR:"rtfOnAir"==document.getElementById("planActivationMethod").value?(c=document.getElementById("departure"),c=getAerodromeByIndex(c.value),a.activation_method=BY_RTF_ON_AIR_STR+c.acc+" "):a.activation_method=BY_TWR;"phoneOnGound"==document.getElementById("planCompletionMethod").value?a.completion_method=BY_PHONE_ON_GROUND_STR:"rtfOnAir"==document.getElementById("planCompletionMethod").value?
(c=document.getElementById("destination"),c=getAerodromeByIndex(c.value),a.completion_method=BY_RTF_ON_AIR_STR+c.acc+" "):a.completion_method=BY_TWR;a.takeoff_time=d.toJSON();a.identification=document.getElementById("aircraftIdentification").value;a.departure=getAerodromeByIndex(document.getElementById("departure").value).icao;a.dep_route=document.getElementById("route_departure_rep").value;d=document.getElementById("route").value;a.route=$("<p>"+d+"</p>").text();a.dest_route=document.getElementById("route_destination_rep").value;
a.destination=getAerodromeByIndex(document.getElementById("destination").value).icao;updateFlyingTimeValues();a.flight_time=gSelectedFlyingTimeStr;return a}function storeCurrentPlan(){var a=getPlanForStoring();addPlanToStorage(a);updateStoredPlanList()}
