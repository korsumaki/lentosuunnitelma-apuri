...
function aerodromeReadyHandler(a){try{var b=a.getElementsByTagName("gpx");0<b.length&&debug_log("Lentokentt\u00e4tiedot p\u00e4iv\u00e4tty: "+b[0].attributes.getNamedItem("created").nodeValue)}catch(c){debug_log("Could not read xml date: "+c)}try{var d=a.getElementsByTagName("aerodrome");OfficialAerodromeArray=[];OfficialAerodromeArray.push(new Aerodrome("Ep\u00e4virallinen",UNOFFICIAL_AERODROME,"000000N","0000000E","no",NOTE_TO_ADD_ACC_FREQ_STR,[]));for(a=0;a<d.length;++a){for(var e=d[a].getElementsByTagName("name")[0].childNodes[0].nodeValue,
...
function updatePlanActivationMethods(a,b){removeOptions(a);"yes"==b.atc&&appendOption(a,BY_TWR,"twr");appendOption(a,"Puhelimella maassa (ACC)","phoneOnGound");appendOption(a,"Radiolla ilmassa (ACC, "+b.acc+")","rtfOnAir")}
...
function updateFlyingTimeValues(){gSelectedFlyingTimeStr=gCalculatedFlyingTimeStr;if(getFlightTimeOverride()){for(var a="",b=document.getElementsByName("flightTimeHour"),c=0;c<b.length;c++)if(!0==b[c].checked){a=b[c].value;break}b=document.getElementsByName("flightTimeMin");for(c=0;c<b.length;c++)if(!0==b[c].checked){a+=b[c].value;break}if(4!==a.length)return alert("Itse m\u00e4\u00e4riteltyyn lentoaikaan t\u00e4ytyy antaa sek\u00e4 tunnit ett\u00e4 minuutit!"),!1;if(Number(a)<Number(gSelectedFlyingTimeStr))return alert("Itse m\u00e4\u00e4ritellyksi lentoajaksi ei voi antaa v\u00e4hemp\u00e4\u00e4 kuin laskelmassa on laskettu!"),
