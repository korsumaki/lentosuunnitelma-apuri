var log="",VfrRepArray,CurrentAerodromeArray,OfficialAerodromeArray,ZZZZFieldArray,gPosition_lat,gPosition_lon,flightPlanLink,GlobalFlyingTimeStr,VFRPortFilename="17NOV2011.xml",AerodromesFilename="aerodromes.xml",ZZZZFieldsFilename="zzzz_fields.xml",UNOFFICIAL_AERODROME="ZZZZ",UNOFFICIAL_AERODROME_INDEX=0,BY_PHONE_ON_GROUND_STR="PHONE ACC 032865172 ",BY_RTF_ON_AIR_STR="RTF ACC ",overrideFlightTime=!0;function debug_log(a){log+=a+"<br>";document.getElementById("log").innerHTML=log}
function getFlightTimeOverride(){return overrideFlightTime}
function setFlightTimeOverride(a){if(overrideFlightTime!=a)if(overrideFlightTime=a,!0==overrideFlightTime)$("#overrideFlightTimeContainer").slideDown();else{$("#overrideFlightTimeContainer").slideUp();var b=document.getElementsByName("flightTimeHour");for(a=0;a<b.length;a++)$("#flightTime"+b[a].value+"h").removeAttr("checked").checkboxradio("refresh");b=document.getElementsByName("flightTimeMin");for(a=0;a<b.length;a++)$("#flightTime"+b[a].value).removeAttr("checked").checkboxradio("refresh")}}
function onFlightTimeOverrideButton(){setFlightTimeOverride(!getFlightTimeOverride())}function isEverythingReady(){var a=!0;void 0==VfrRepArray&&(a=!1);void 0==OfficialAerodromeArray&&(a=!1);void 0==ZZZZFieldArray&&(a=!1);return a}function continueIfEverythingIsReady(){isEverythingReady()&&(PopulateSelections(),onChangeDepartureAd(),onChangeDestinationAd());void 0!==OfficialAerodromeArray&&(void 0!==ZZZZFieldArray&&void 0!==gPosition_lat)&&handlePositionUpdate(gPosition_lat,gPosition_lon)}
...
f=d[a].getElementsByTagName("icao")[0].childNodes[0].nodeValue,g=d[a].getElementsByTagName("atc")[0].childNodes[0].nodeValue,h=d[a].getElementsByTagName("acc")[0].childNodes[0].nodeValue.replace("."," "),j=d[a].attributes.getNamedItem("lat").nodeValue,k=d[a].attributes.getNamedItem("lon").nodeValue,l=d[a].getElementsByTagName("vfrpoint"),b=[],m=0;m<l.length;++m)b.push(l[m].childNodes[0].nodeValue);OfficialAerodromeArray.push(new Aerodrome(e,f,j,k,g,h,b))}}catch(n){alert(n)}debug_log("=> "+OfficialAerodromeArray.length+
...
h=d[a].getElementsByTagName("acc")[0].childNodes[0].nodeValue.replace("."," "),j=d[a].attributes.getNamedItem("lat").nodeValue,k=d[a].attributes.getNamedItem("lon").nodeValue;d[a].getElementsByTagName("vfrpoint");ZZZZFieldArray.push(new Aerodrome(e,f,j,k,g,h,[]))}}catch(l){alert(l)}debug_log("=> "+ZZZZFieldArray.length+" ep\u00e4virallista lentopaikkaa.");continueIfEverythingIsReady()}function removeOptions(a){for(;0<a.length;)a.remove(0)}
...
function isAllSettingsAvailable(){try{var a=document.getElementById("aircraftIdentification").value,b=document.getElementById("aircraftType").value,c=document.getElementById("equipment").value,d="ok";isPbnNeeded()&&(d=document.getElementById("equipmentPBN").value);var e=document.getElementById("ssr").value,f=document.getElementById("aircraftSpeedUnit").value,g=document.getElementById("aircraftSpeed").value,h=document.getElementById("pilot").value,j=document.getElementById("pilotTel").value,k=document.getElementById("aircraftColor").value;
return void 0===GlobalFlyingTimeStr?!1:isValid(a)&&isValid(b)&&isValid(c)&&isValid(e)&&isValid(d)&&isValid(f)&&isValid(g)&&isValid(h)&&isValid(j)&&isValid(k)?!0:!1}catch(l){return!1}}function onLoad(){loadXMLDoc(AerodromesFilename,aerodromeXmlResponseHandler);navigator.geolocation?updateCurrentLocation():(debug_log("Geolocation ei ole tuettu selaimessa."),fakeCurrentLocation());updateFromLocalStorage();onChangeFlightLv()}
...
function fillVfrWaypointSelectionData(a,b,c){removeOptions(a);var d=getVfrWayPointsByName(b.vfrPoints);if(0!=d.length){sortNearestFirst(d,c.lat,c.lon);appendOption(a,"-","");for(var e=0;e<d.length;++e)appendOption(a,d[e].name,d[e].name);b!==c&&(a.selectedIndex=1)}}function updatePlanActivationMethods(a,b){removeOptions(a);"yes"==b.atc&&appendOption(a,"- (torni)","twr");appendOption(a,"Puhelimella maassa (EFES)","phoneOnGound");appendOption(a,"Radiolla ilmassa (EFES, "+b.acc+")","rtfOnAir")}
...
d,d=a[g];d="<tr><th>Yhteens\u00e4</th>";d+="<th>"+Math.round(getDistanceInCurrentUnit(f))+" "+c+"</th>";d+="<th>"+getTimeFromDistance(f,b)+" min</th>";d+="</tr>";e+=d;document.getElementById("flyingTime").innerHTML=e;b=getTimeFromDistance(f,b);b=timeNext5mins(b);10>b&&(b=10);GlobalFlyingTimeStr=convertFlyingTimeToString(b);setFlightTimeOverride(!1)}}
...
a=GlobalFlyingTimeStr;if(getFlightTimeOverride()){for(var d="",e=document.getElementsByName("flightTimeHour"),c=0;c<e.length;c++)if(!0==e[c].checked){d=e[c].value;break}e=document.getElementsByName("flightTimeMin");for(c=0;c<e.length;c++)if(!0==e[c].checked){d+=e[c].value;break}if(4!==d.length)return alert("Lentoaikaan t\u00e4ytyy antaa sek\u00e4 tunnit ett\u00e4 minuutit!"),!1;if(Number(d)<Number(a))return alert("Lentoajaksi ei voi antaa v\u00e4hemp\u00e4\u00e4 kuin laskelmassa on laskettu!"),!1;
a=d}b+="&eet="+a;c="";a=document.getElementById("departure").value;a==UNOFFICIAL_AERODROME_INDEX?(c+="DEP%252F"+document.getElementById("zzzz_departure").value+" ",c+=CurrentAerodromeArray[UNOFFICIAL_AERODROME_INDEX].latStr+" ",c+=CurrentAerodromeArray[UNOFFICIAL_AERODROME_INDEX].lonStr+" "):(a=getAerodromeByIndex(a),a.icao==UNOFFICIAL_AERODROME&&(c+="DEP%252F"+a.name+" ",c+=a.latStr+" ",c+=a.lonStr+" "));a=document.getElementById("destination").value;a==UNOFFICIAL_AERODROME_INDEX?c+="DEST%252F"+
document.getElementById("zzzz_destination").value+" ":(a=getAerodromeByIndex(a),a.icao==UNOFFICIAL_AERODROME&&(c+="DEST%252F"+a.name+" ",c+=a.latStr+" ",c+=a.lonStr+" "));isPbnNeeded()&&(a=document.getElementById("equipmentPBN").value,c+="PBN%252F"+a+" ");c+="RMK%252F";"phoneOnGound"==document.getElementById("planActivationMethod").value?c+="DEP "+BY_PHONE_ON_GROUND_STR:"rtfOnAir"==document.getElementById("planActivationMethod").value&&(a=document.getElementById("departure"),a=getAerodromeByIndex(a.value),
c+="DEP "+BY_RTF_ON_AIR_STR+a.acc+" ");"phoneOnGound"==document.getElementById("planCompletionMethod").value?c+="ARR "+BY_PHONE_ON_GROUND_STR:"rtfOnAir"==document.getElementById("planCompletionMethod").value&&(a=document.getElementById("destination"),a=getAerodromeByIndex(a.value),c+="ARR "+BY_RTF_ON_AIR_STR+a.acc+" ");c+="PIC TEL "+document.getElementById("pilotTel").value;b+="&other="+c;a="";d=document.getElementsByName("enduranceHour");for(c=0;c<d.length;c++)if(d[c].checked){a=d[c].value;break}d=
document.getElementsByName("enduranceMin");for(c=0;c<d.length;c++)if(d[c].checked){a+=d[c].value;break}if(0===Number(a))return alert("Toiminta-aika ei voi olla nolla!\nMeinasitko p\u00e4\u00e4st\u00e4 pitk\u00e4lle?"),!1;b+="&endurance="+a;a="";d=document.getElementsByName("persons");for(c=0;c<d.length;c++)if(d[c].checked){a=d[c].value;break}b+="&person="+a;b+="&markings="+document.getElementById("aircraftColor").value;b+="&pic="+document.getElementById("pilot").value;b+="&tel="+document.getElementById("pilotTel").value;
b+="&filed="+document.getElementById("pilot").value;b=convertScandinavianLetters(b);flightPlanLink="http://www.finavia.fi/C/flightplan/efpl_lomake/"+b;return!0}
function onClickFlightPlanLinkButton(){if(isAllSettingsAvailable())try{updateFlightPlanLink()&&window.open(flightPlanLink)}catch(a){alert("Lentosuunnitelman luominen ei onnistunut.\n"+a)}else alert("Perustietoja puuttuu viel\u00e4.\nLis\u00e4\u00e4 puuttuvat tiedot Perustiedot sivulle, niin lentosuunnitelma saadaan t\u00e4ytetty\u00e4 oikein."),window.open("#page-plan-settings","_self")};