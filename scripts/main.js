...
/*
 * TODO list:
 * + kokeile vfr tiedon asetusta (t‰yt‰ seuraava kentt‰ oikein)
 * + jos reitti on tyhj‰, lis‰‰ space (vai nolla?)
 * + Kohdassa 18 edelleen "DEF RTF EFES ACC"; pelkk‰ "DEP RTF ACC" riitt‰isi
 * + Jos perustiedot-osassa on nopeusyksikˆksi annettu KT, olisi luontevaa n‰ytt‰‰ kokonaismatka myˆs NM, ei KM
 * + myˆs RMK/ kauttaviiva muutettu escape koodiksi
 * + myˆs kentt‰listaukseen km -> NM
 * + Joissakin koneissa on enemm‰n kuin 4 paikkaa
 * + VFR tiedon tilalle muitakin vaihtoehtoja A000, F000, ...
 * + Olisiko mahdollista saada useamman koneen tuki ilman ett‰ perustiedoista pit‰isi k‰yd‰ muuttamassa koneen tietoja?
 * + l‰htˆaika samoin kuin toiminta-aika, 15 min jaolla
 * + molempiin aikoihin 1h lis‰‰?
 * + Jos koneessa on laitteissa "G" , ts BRNAV-hyv‰ksytty GPS (kuten GNS430/530) tulisi kohtaan 18 t‰ytt‰‰ aina lis‰ksi PBN/xxx  (ks. taustat t‰‰lt‰ http://www.rocketroute.com/ICAO2012-PBN-RNAV.php )
 *     + jos laitteissa on R -> avaa uusi kentt‰ "PBN/jotain"
 *     + 18 kent‰n t‰yttˆ lis‰‰ uuden kent‰n sis‰ltˆ ennen RMK/ kohtaa
 *     + tietojen tallennus ja lataus
 *     + lis‰‰ pbn tiedon tarkistus vaadittuihin tietoihin ennen lomakkeelle siirtymist‰
 * + lentolaskelmataulukon yhteyteen valitun koneen tunnus n‰kyviin
 * + zzzz kent‰n koordinaatit 4+5 numerolla. ConvertToDM()
 * + lentoajan antaminen itse (paikallislennoille tai jos muuten haluaa kierrell‰ enemm‰n)
 * -
 * 
 * /- lis‰‰ tallenna -nappi
 * - tallennetaan plaanin tiedot (localstorageen) (ilman virallista plaanin j‰tt‰mist‰)
 * 		+- tallenna storageen
 * 			- ep‰virallisen nimi + koordinaatit?
 * 		+- lue storagesta ja muodosta plaanilista etusivulle
 * 			- (ep‰virallisen) kent‰n nimi
 * 			- kaikki tiedot n‰kyvyll‰ (tai t‰p‰tt‰viss‰ n‰kyviin), ei linkki‰
 ...
 * 		+ plaanin deletointi
 * 		+ lis‰‰ tietojen tallennusta
 * 			+ zzzz kent‰n selv‰kielinen nimi
 * 			+ korkeus
 * 			+ toiminta-aika
 * 			+ hlˆ m‰‰r‰
 * - k‰yttˆohjesivu
...
function handlePositionUpdate(lat, lon) {
	...
	if (document.getElementById("zzzzFieldsEnabled").value == "enabled") {
		debug_log("Kaikki lentokent‰t k‰ytˆss‰.");
		CurrentAerodromeArray = OfficialAerodromeArray.concat(ZZZZFieldArray);
	} else {
		debug_log("Vain viralliset lentokent‰t k‰ytˆss‰.");
...
function updateStoredPlanList() {
	...
	for (var i=0; i<storedPlans.length; i++) {
		...
		var text = "<li class='storedPlanListItem'>" //<a href='#'>
			...
			+ "<p class='ui-li-aside'>L‰htˆaika: <strong>" + timeStr +"</strong> (sa)</p>"
		...
		text += "</li>";
			...
			//+ "</p>"
			//+ "</div>"
			//+ "<a href='#page-stored-plan'>N‰yt‰ lentosuunnitelma</a></li>";
...
function createFlyingTimeTable() {
	...
	if (aircraftSpeed == 0) {
		document.getElementById("flyingTime").innerHTML = '<a href="#page-plan-settings" class="ui-btn-right">T‰yt‰ Perustiedot sivulle puuttuvat tiedot!</a>';
		
		//debug_log("T‰yt‰ Perustiedot sivulle puuttuvat tiedot!");
		return;
	}	
...
function updateFlyingTimeValues() {
	...
	if (getFlightTimeOverride()) {
		...
		if (flightTimeOverrideStr.length !== 4) {
			alert("Lentoaikaan t‰ytyy antaa sek‰ tunnit ett‰ minuutit!");
			return false;
		}
		if (Number(flightTimeOverrideStr) < Number(gSelectedFlyingTimeStr)) {
			alert("Lentoajaksi ei voi antaa v‰hemp‰‰ kuin laskelmassa on laskettu!");
			return false;
		}
