...
function aerodromeReadyHandler(xmlDoc) {
	try {
		...
		if (gpx_list.length >0) {
			debug_log( "Lentokenttätiedot päivätty: " + gpx_list[0].attributes.getNamedItem("created").nodeValue);
		}
	} catch(err) {
		debug_log("Could not read xml date: " + err);
	}
...
function zzzzFieldReadyHandler(xmlDoc) {
	...
	try {
		var gpx_list = xmlDoc.getElementsByTagName("gpx");
		if (gpx_list.length >0) {
			debug_log( "Epävirallisten lentopaikkojen tiedot päivätty: " + gpx_list[0].attributes.getNamedItem("created").nodeValue);
		}
	} catch(err) {
		debug_log("Could not read xml date: " + err);
	}
